...
interface CardWrapperProps extends HTMLAttributes<HTMLDivElement> {
  ...
  hover?: boolean;
  selected?: boolean;
  variant?: 'default' | 'glass' | 'outlined' | 'elevated' | 'glass-80';
...
export default function CardWrapper({
  ...
  const hoverClasses = hover ? 'hover:shadow-xl hover:border-po1 cursor-pointer transition-all duration-300' : '';
  const selectedClasses = selected ? 'border-po1 ring-2 ring-po1 ring-opacity-20' : '';

  return (
    <div
      className={`${baseClasses} ${variantClasses[variant]} ${paddingClasses[padding]} ${roundedClasses[rounded]} ${hoverClasses} ${selectedClasses} ${className}`}
      {...props}
    >
      {children}
    </div>
