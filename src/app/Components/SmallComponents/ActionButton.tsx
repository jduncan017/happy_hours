...
export default function ActionButton({
  ...
  const variantClasses = {
    primary: theme === 'dark' 
      ? 'bg-gradient-to-r from-po1 to-py1 text-white hover:shadow-xl transform hover:scale-[1.02] focus:ring-po1/50' 
      : 'bg-gradient-to-r from-po1 to-py1 text-white hover:shadow-xl transform hover:scale-[1.02] focus:ring-po1/50',
  ...
  const disabledClass = (disabled || loading) ? 'opacity-60 cursor-not-allowed transform-none hover:scale-100' : '';
  const ringOffset = theme === 'dark' ? 'focus:ring-offset-stone-900' : 'focus:ring-offset-white';
