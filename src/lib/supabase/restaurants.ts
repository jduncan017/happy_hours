...
function transformDatabaseRowToRestaurant(
  ...
  return {
    ...
    coordinates: row.coordinates
      ...
      : undefined,
    area: row.area && row.area.trim() !== "" ? row.area : "Location TBD",
    cuisineType:
      row.cuisine_type && row.cuisine_type.trim() !== ""
        ? row.cuisine_type
        : "Cuisine TBD",
...
export async function getAllRestaurants(
  ...
  return data.map((row) => {
    ...
    if (
      row.restaurant_ratings &&
      Array.isArray(row.restaurant_ratings) &&
      row.restaurant_ratings.length > 0
    ) {
...
export async function updateRestaurant(
  ...
  if (updates.coordinates !== undefined) {
    updateData.coordinates = updates.coordinates
      ? [updates.coordinates.lng, updates.coordinates.lat]
      : null;
  }
  if (updates.area !== undefined) updateData.area = updates.area;
  if (updates.cuisineType !== undefined)
    updateData.cuisine_type = updates.cuisineType;
  if (updates.priceCategory !== undefined)
    updateData.price_category = updates.priceCategory;
  ...
  if (updates.happyHours !== undefined)
    updateData.happy_hours = updates.happyHours;
